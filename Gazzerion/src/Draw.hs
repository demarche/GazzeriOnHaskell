...
drawPlayerCards world = do
    let csize = world^.cardsizeMax
        maxh = world^.maxhandcards
        maxm = world^.maxmagiccards
        maxp = world^.maxplayer
        ssize = world^.screensize
    ...
    forM_ [0..maximum [3, maxp-1]] $ \i ->
        case i `mod` 4 of
            0 -> do
                ...
                translate (v2Int (myx + env^.grid) (myy + env^.grid)) $ drawCardBack world
                color white $
                 translate (v2Int (myx + env^.grid * (1 + csize^.width `div` 2)) (myy + env^.grid * (1 + csize^.height `div` 2)) - charhlf) $ text (world^.font) charsize (deckstr i)
                forM_ [0..length(hcards!!i)-1] $ \x -> translate (v2Int (myx + env^.grid * (1 + csize^.width * (1 + x))) (myy + env^.grid)) $ drawCard (hcards!!i!!x) env -- 手札描画
            _ -> line[V2 0 0, V2 0 0]